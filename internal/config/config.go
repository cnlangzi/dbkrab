...
type DBConfig struct {
	...
	Logs   string `yaml:"logs"`   // Path for logs DB (pull_logs, skill_logs, sink_logs)
}
...
func Load(path string) (*Config, error) {
	...
	if cfg.App.DB.CDC == "" {
		cfg.App.DB.CDC = "./data/app/cdc.db"
	}
	if cfg.App.DB.Offset == "" {
		cfg.App.DB.Offset = "./data/app/offset.db"
	}
	if cfg.App.DB.DLQ == "" {
		cfg.App.DB.DLQ = "./data/app/dlq.db"
	}
	if cfg.App.DB.Logs == "" {
		cfg.App.DB.Logs = "./data/app/logs.db"
	}
	if cfg.App.Listen == 0 {
		cfg.App.Listen = 9020
	}
